...
def Ultimate(data_train, data_valid):
    ...
    u0 = classe0[["x1", "x2"]].mean().values
    u1 = classe1[["x1", "x2"]].mean().values
    cov_0 = np.cov(classe0[["x1", "x2"]].values.T)
    cov_inv0 = np.linalg.inv(cov_0)
    cov_1 = np.cov(classe1[["x1", "x2"]].values.T)
    cov_inv1 = np.linalg.inv(cov_1)
    p_c0 = len(classe0) / len(df_train)
    p_c1 = len(classe1) / len(df_train)

    def mahalanobis_distance(x, u, cov_inv, p):
        delta = x - u
        distance = (delta.T @ cov_inv @ delta) - np.log(np.linalg.det(cov_inv)) - 2*np.log(p)
        return distance
    
    def prediction(x):
        dist0 = mahalanobis_distance(x, u0, cov_inv0, p_c0)
        dist1 = mahalanobis_distance(x, u1, cov_inv1, p_c1)
    ...
    plt.title('Frontière de décision avec la distance de mahalanobis')
    plt.xlabel('x1')
    plt.ylabel('x2')
    plt.legend()
    plt.show()
